...
def compute_raoq(sample: str) -> xr.DataArray:
    ...
    ds_tr = thresholds_modis.sel(quantile=0.20, location=pca_projection.location.values)
...
def compute_kl_div(sample: str) -> xr.DataArray:
    # Load datasets
    ...
    kl_div.coords["raoq"] = group_da

    def mean_kl_func(x, axis=None):
        return np.nanmean(x, axis=axis)

    mean_kl = kl_div.groupby("raoq").reduce(mean_kl_func, dim="location")
    # kl_div_array = grouped_variance.reindex(raoq=unique_values)

    # Convert to dataset and save
    mean_kl_ds = mean_kl.to_dataset(name="kl_div")
    save_path = f"/Net/Groups/BGI/scratch/crobin/PythonProjects/ExtremesProject/experiments/2025-04-14_12:48:54_full_fluxnet_therightone_highveg/EVI_EN/{sample}/kl_div_raoq.zarr"
...
if __name__ == "__main__":
    ...
    def process_sample(sample):
        try:
            ...
            # if "variance.zarr" not in os.listdir(sample_path):
            compute_raoq(sample)
